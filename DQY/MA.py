...
def judge(means, cur_price, offset, commission): ##MODIFY
    if abs(offset + ((means - cur_price)/means)) <= commission :
        return 0
    else :
...
def calR_M(date, time, Assert_rate): ##MODIFY
    if date < time:
#         print(np.mean(Assert_rate[0 : date + 1]))
        return np.mean(Assert_rate[0 : date + 1])
        
    else:
#         print(np.mean(Assert_rate[date - time + 1 : date + 1]))
        return np.mean(Assert_rate[date - time + 1 : date + 1])

#----------------------------------------------------------------------

def S_MA(customer, bitcoin, time_sort, gold, time, date, buy_days, buy_days_gold):
    ###
    bit_rate = pd.read_csv('DQY/offset_bit.csv')
    bit_rate = bit_rate['比特币价格日增幅']
    gold_rate = pd.read_csv('DQY/offset_gold.csv')
    gold_rate = gold_rate['黄金价格日增幅']
    ###
    if date in time_sort:
            ...
            ###
            offset_bit = calR_M(date, 2, bit_rate)
            offset_gold = calR_M(date, 1, gold_rate)
            ###
            j1 = judge(means_bitcoin, bitcoin[date], offset_bit, PARAM.BITCOIN_PRE)   ##MODIFY
            j2 = judge(means_gold, gold[gold_date_index], offset_gold, PARAM.GOLD_PRE) ##MODIFY
            mode1 = 2
            mode2 = 2
            if j1 > 0 and j2 > 0:
                mode1 = 0
